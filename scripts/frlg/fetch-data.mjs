 ...
 *
 * Run: node scripts/frlg/fetch-data.mjs
 * Output: src/games/frlg/pokemon.json
 */

import fs from 'fs'
import path from 'path'
import { fileURLToPath } from 'url'
import { manualData, versionExclusives, nationalEvolutionIds, gen3TypeOverrides, frlgMachines } from '../../src/games/frlg/manual.js'

const __dirname = path.dirname(fileURLToPath(import.meta.url))
const OUTPUT_PATH = path.join(__dirname, '../../src/games/frlg/pokemon.json')
const DELAY_MS = 200
...
async function main() {
  ...
  const evoSrc = fs.readFileSync(
    new URL('../../src/games/frlg/evolutions.js', import.meta.url).pathname, 'utf-8'
  )
